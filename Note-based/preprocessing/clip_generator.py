...
def process_and_save_clip(npy_file, csv_file, index):
    ...
    output_file = os.path.join(
        instrument_dir, f"{os.path.splitext(os.path.basename(npy_file))[0]}_{index}.h5"
    )
    ...
    with h5py.File(output_file, "w") as hf:
        ...
        hf.create_dataset(
            "instrument", data=np.array([instrument])
        )  # 스칼라 값을 배열로 변환하여 저장
        hf.create_dataset(
            "note", data=np.array([note])
        )  # 스칼라 값을 배열로 변환하여 저장
        hf.create_dataset(
            "start_time", data=np.array([new_start_time])
        )  # 스칼라 값을 배열로 변환하여 저장
        hf.create_dataset(
            "end_time", data=np.array([new_end_time])
        )  # 스칼라 값을 배열로 변환하여 저장

    print(
        f"Saved clip {output_file} with instrument {instrument_name} and note {note} "
